<script lang="ts" module>
  import type { AgentActionMessage, Message } from "$lib/tauri/types";

  interface PromptGroup {
    prompt: string;
    timestamp: string;
    actions: AgentActionMessage[];
  }

  function groupByPrompt(messages: Message[]): PromptGroup[] {
    const groups: PromptGroup[] = [];
    let current: PromptGroup | null = null;

    for (const msg of messages) {
      if (msg.role === "user") {
        current = { prompt: msg.content, timestamp: msg.timestamp, actions: [] };
        groups.push(current);
      } else if (msg.role === "agent" && msg.type === "action" && current) {
        current.actions.push(msg);
      }
    }
...
<script lang="ts">
  ...
  let groups = $derived(groupByPrompt(chatStore.messages));
  let totalActions = $derived(groups.reduce((n, g) => n + g.actions.length, 0));
...
<div class="flex flex-col h-full">
  <div class="px-3 py-2 text-[10px] text-[--color-text-dim] uppercase tracking-widest border-b border-[--color-border] flex items-center gap-2">
    ...
    {/if}
    {#if totalActions > 0}
      <span class="ml-auto text-[--color-text-dim] normal-case tracking-normal">{totalActions}</span>
    {/if}
  </div>

  <div class="flex-1 overflow-y-auto" use:autoscroll>
    {#if groups.length === 0 && !isWorking}
    ...
    {:else}
      {#each groups as group, gi (gi)}
        <div class="border-b border-[--color-border]">
          <!-- Prompt header -->
          <div class="px-3 py-2 flex items-center gap-2 bg-[--color-bg-surface]/50">
            <span class="text-[10px] text-[--color-accent] shrink-0">❯</span>
            <span class="text-[11px] text-[--color-text] truncate flex-1">{group.prompt}</span>
            <span class="text-[9px] text-[--color-text-dim] shrink-0">{group.actions.length}</span>
            <span class="text-[9px] text-[--color-text-dim] shrink-0">{group.timestamp}</span>
          </div>

          <!-- Actions under this prompt -->
          {#each group.actions as msg (msg.id)}
            ...
            <div class="border-t border-[--color-border]/30">
              <div class="px-3 py-1 flex items-center gap-2 pl-6">
